...
#(3) Base de Datos
#-----------------------------------------
load(paste0(dirmother,"/Base/Base_Filtradas/02_Base_filtrada_240.RData"))
# base = BD.1 %>% select(c(1:7,13,8:10,12,14:15))%>% filter(ID %in% pl$ID) 
# rm(BD.1,pl)

#Identificar los items de la base inicial que no han producido cambio alguno
# source(paste0(dirmother,"/contarcambios.R"))
# tabla = contarcambios(base)
# tabla = filter(tabla, ncambios != 0) 
#Eliminamos aquellos items que no han cambiado a lo largo del tiempo
#base_p = filter(base, ID %in% tabla$ID)
#ESTAMOS PERDIENDO INFORMACION

#Completamos los dias vacios
source(paste0(dirmother,"/completardias.R"))
...
fn = paste0(dirmother,"/Base/Base_Final_Peru_alterada_01.RData") #guardamos base
save(base_c, file = fn)
...
#Completamos precios
source(paste0(dirmother,"/llenarnas.R"))
source(paste0(dirmother,"/generalizar.R"))
...
source(paste0(dirmother,"/EncuentraDuraciones.R"))
source(paste0(dirmother,"/generalizar2.R"))
...
#Incorporacion de dummies de fechas de oferta:
...
"Guardamos"
fv_save = paste0(dirmother,"/Base/Base_duraciones_01.RData")
