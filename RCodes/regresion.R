regresion <- function(baseduracion,basetipocambio,j,s,y){
  ...
  #Regresion OLS
  #--------------
  reg1 <- lm(Dpre ~ 0 + d_ER  + ER_freq +demeanedfreq+
               control + categoria,
             data = baseduracion)
  
  #Regresion FGLS
  #--------------
  reg2 <- lm(Dpre ~ 0 + d_ER   + ER_freq + demeanedfreq +
                control + categoria, data = baseduracion,
