...
#---------------------------
BASEDATOS$log_precio_nor = log(BASEDATOS$precio_normal_completo)
BASEDATOS$magnitud[2:nrow(BASEDATOS)] = (BASEDATOS$log_precio_nor[2:nrow(BASEDATOS)]-BASEDATOS$log_precio_nor[1:nrow(BASEDATOS)-1])*100
...
cambios_dia$magnitud_cambio_promedio[is.nan(cambios_dia$magnitud_cambio_promedio)] <- NA
#******************************************
...
ggplot(data = cambios_dia ) + geom_point(aes(x = fecha, y = mediana),na.rm=TRUE, col = "blue") + 
  ...
  annotate("text", x = xdate, y = 20, label = 19.43, color = "red")
